...
```{r}
library(tidyr)
DF3convert <- na.omit(DF3)
pivot_wider(DF3convert, lesson_name, attempt)
```
...
```{r}
DF5 <- select(DF1, 'hash', 'lesson_name', 'correct')
...
DF6 <- na.omit(DF6)
DF6 = DF6 %>% group_by(datetime) %>% summarise(average=mean(correct))
```
