...
V2.TCH <- EDGE %>% group_by(from) %>% summarise(sum(count))
names(V2.TCH) = c("character","count")

V2.STD <- EDGE %>% group_by(to) %>% summarise(sum(count))
names(V2.STD) = c("character","count")


VERTEX2 <- bind_rows(V2.TCH, V2.STD)
...
##Use*5 to emphasize the size difference
...
##Combine name
D3 <- D3 %>% unite(Name, First.Name, Last.Name, remove = TRUE, sep=" ")
D3<-D3[-1,]
...
V(m)$cex<-0.5
plot(m,layout=layout.fruchterman.reingold,vertex.color=rainbow(7),vertex.label.cex=0.4)
```
...
##Yujun Zhang and Lintong Li are the most central people!
```
