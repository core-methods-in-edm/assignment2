...
plot(g2, layout = layout.fruchterman.reingold, vertex.color = VERTEX2$gender,vertex.size = VERTEX2$count+10, arrow.size=0.3)
```
...
data1<-unite(data1, StudentName, "First.Name", "Last.Name", sep=" ")
# gather & select required data
data2 <- data1 %>% gather (key = classnum, value = class, Class.1:Class.6)  %>% select (c(-classnum))

data2 <- filter(data2, class >0)
data2 <- filter(data2, class != "HUDK4050")
data2$count<-1
data2<-unique(data2)
data3 <- data2 %>% spread(class, count) 

# create matrix
row.names(data3)<-data3$StudentName
data3$StudentName<-NULL
data3<-as.matrix(data3)
data3 = ifelse(is.na(data3), 0, 1)
data4<-data3 %*% t(data3)
diag(data4)<-NA
# graph
g3<-graph.adjacency(data4, mode="undirected")
