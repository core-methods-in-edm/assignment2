...
```{r}
student_count<- EDGE %>% group_by(to) %>% summarise(count= sum(count))
teacher_count <- EDGE %>% group_by(from) %>% summarise(count= sum(count))
names(student_count) <- c("id","count")
names(teacher_count) <- c("id","count")
total_count <- bind_rows(student_count,teacher_count)
...
class_2017 <- read.csv("HUDK4050_2017_SNA_classes.csv", header = TRUE)
DF_class <- select(class_2018,Q8,Q9,Q1,Q3,Q4,Q5)
