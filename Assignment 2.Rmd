...
S2 = data.frame(score = rep(100, 100 - length(S1[,1])))
S3 = rbind(S1,S2)
#round rounds numbers to whole number values
S3 = round(S3)
#sample draws a random samples from the groups vector according to a uniform distribution
groups = sample(c('sport', 'music', 'nature', 'literature'), 100, replace = TRUE)
sample = data.frame(score, groups)
```
...
(rcut = cut(score, 15, labels = letters[1:15]))
```
...
cv = c(brewer.pal(4, "Accent"), brewer.pal(11, "Set3"))

#Use named palette in histogram
cgroups = groups
cgroups[cgroups[] == "literature"] = cv[1]
cgroups[cgroups[] == "sport"] = cv[2]
cgroups[cgroups[] == "music"] = cv[3]
cgroups[cgroups[] == "nature"] = cv[4]
sample$colors = cgroups
hist(sample$score, col = cv, breaks = 15)
```
...
boxplot(sample$score~sample$groups, col = cv)
```
...
```{r}
nlogin = rnorm(100, 15, 5)
df = data.frame(nlogin)
df = filter(df, nlogin<=25 & nlogin>=1)
df1 = data.frame(nlogin = rep(20, 100 - length(df[,1])))
df2 = rbind(df,df1)
...
```{r}
plot(df2$nlogin, sample$score,col = cgroups, main = "the relationships between logins and scores")                             
```
...
DF2 = cbind(DF1["hash"], DF1["lesson_name"], DF1["attempt"])
(DF3 <- DF2 %>% 
    group_by(lesson_name, hash) %>% 
    summarise(sum_attempt = sum(attempt), .groups = 'drop')  
  )
```
...
```{r}
DF3_LN = DF1['lesson_name'][!duplicated(DF1['lesson_name']),]
DF3_H = DF1['hash'][!duplicated(DF1['hash']),]
x = matrix(NA, nrow = length(DF3_H), ncol = length(DF3_LN), dimnames = list(DF3_H, DF3_LN))
for (r in 1:length(DF3$lesson_name)){
  a = DF3[r,,]$lesson_name
  if(a == ''){
   a = 15
  }
  x[toString(DF3[r,,]$hash), a] = DF3[r,,]$sum_attempt
}
DF3C = data.frame(x)
```
...
```{r}
DF4 = cbind(DF1["hash"], DF1["lesson_name"], DF1["correct"])
DF4$correct[is.na(DF4[,3])] = 0

DF4$correct[DF4[,3]=="TRUE"] = 1
DF4$correct[DF4[,3]=="FALSE"] = 0
...
```{r}
DF5B = cbind(DF1["hash"], DF1["course_name"], DF1["correct"])
DF5B$correct[is.na(DF5B[,3])] = 0
DF5B$correct[DF5B[,3]=="TRUE"] = 1
DF5B$correct[DF5B[,3]=="FALSE"] = 0
DF5B$correct = as.numeric(DF5B$correct)
DF5B$correct[is.na(DF5B[,3])] = 0 
(DF5 <- DF5B %>% 
    group_by(hash, course_name) %>% 
    summarise(mean.correct = mean(correct), .groups = 'drop')
  )
```
