...
```{r}
library(tidyr)
teacher<-dplyr::group_by(EDGE[,c(1,3)], from)
t1<-teacher %>% group_by(from) %>% summarise(sum(count))
names(t1)<-c("id","size")
student<-dplyr::group_by(EDGE[,c(2,3)], to)
s1<-student %>% group_by(to) %>% summarise(sum(count))
names(s1)<-c("id","size")
bind<-bind_rows(t1,s1)
VERTEX<-left_join(VERTEX,bind,by="id")
plot(g,layout=layout.fruchterman.reingold,vertex.color=VERTEX$gender,vertex.size=3*VERTEX$size)
```
...
D3$registered<-1
D3<-D3[,-c(2,3)]
D4<-gather(D3,course,course_name,2:6,na.rm=TRUE)
D4<-D4[D4$course_name!="",]
D4<-D4[,c(1,2,4)]
D5<-spread(D4,key=course_name,value=registered)
D5[is.na(D5)]<-0
D5<-D5[,-1]
row.names(D5)<-D3$student
M1<-as.matrix(D5)
M2<-t(D5)
M3<-M1 %*% M2
M4<-M2 %*% M1
...
```{r}
library(igraph)
g <- graph.adjacency(t(M3), mode="directed")
plot(g,vertex.size=3,edge.width=0.005)
k <- graph.adjacency(t(M4), mode="directed")
plot(k)
```
