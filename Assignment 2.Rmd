...
edge1 <- count(D2, stid, tid)
names(edge1) <- c("receive", "from", "counts")
edge1



#First we will separate the teachers from our original data frame
TCH <- select(D1, tid, t.gender, t.expertise)
#Remove all the repeats so that we just have a list of each teacher and their characteristics
TCH <- unique(TCH)
#Add a variable that describes that they are teachers
TCH$group <- "teacher"
...
#Now repeat this process for the students
STD <- select(D1, stid, s.gender, s.major)
STD <- unique(STD)
STD$group <- "student"

#Make sure that the student and teacher data frames have the same variables names
names(TCH) <- c("id", "gender", "topic", "group")
names(STD) <- c("id", "gender", "topic", "group")

#Bind the two data frames together (you will get a warning because the teacher data frame has 5 types of id (A,B,C,D,E) and the student has 25 (1-30), this isn't a problem)
VERTEX2 <- bind_rows(TCH, STD)
VERTEX2
g <- graph.data.frame(edge1, directed=TRUE, vertices=VERTEX2)
plot(g, layout=layout.circle,  vertex.color=VERTEX2$gender, edge.width=edge1$counts)
...
```{r}
###clean data
### keep course number in the same format
### combine first name &last name
###keep names, course numbers (each person has several courses in each row)
###set edge(name)
####nodes as course numbers
