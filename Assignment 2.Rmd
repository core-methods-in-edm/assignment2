# Assignment 2 - Social Network Analysis  ###Chengxuan Hu
...
```{r}
EDGESTD <- count(D2, stid)
names(EDGESTD) <- c("id","count")
EDGETCH<- count(D2,tid)
names(EDGETCH) <- c("id","count")
total<- bind_rows(EDGESTD,EDGETCH)
VERTEX2<-full_join(VERTEX, total, by="id")
g2 <- graph.data.frame(total, directed=TRUE, vertices=VERTEX2)
plot(g2,layout=layout.fruchterman.reingold, vertex.color=VERTEX2$gender, vertex.size=VERTEX2$count+10)
```
...
D3<-select(D3,Q8,Q9,Q1,Q3,Q4,Q5,Q6,Q7)
### combine name
D4<- D3 %>% unite("Name",Q8:Q9, sep=" ", remove=FALSE)
D5<-select(D4,Name,Q1,Q3,Q4,Q5,Q6,Q7)
D6<-drop(D5[3:55,])
attach(D6)
```
...
```{r}
### gather
D7<- gather(D6,classnumber,value=class,2:7)
### modify classes
D7$class<-gsub("[a[[:space:]]","",D7$class)
D7$class[67]<-"IFSF4090"
D7$class[120]<-"EDPS4002"
D7$class[34]<-"HUDK4050"
D7$class[87]<-"HUDM4125"
D7$class[140]<-"HUDM5026"
D7$class[193]<-"HUDM5126"
D8<-D7[which(D7$class!=""),]
D9<-D8[,-2]
D9<-D9 %>% filter(class!="HUDK4050")
### mutate & spread
D10<-mutate(D9,count=1)
attach(D10)
D11<-spread(D10,key=class,value=count)
D11[is.na(D11)]<-0
row.names(D11)<-D11$Name
D11<-D11[,-1]
### person-class matrix
D11
```
...
```{r}
### graph
g3<-graph.adjacency(D13,mode="undirected")
...
which(degree(g3)==max(degree(g3)))  ## Lintong Li
which(betweenness(g3)==max(betweenness(g3)))   ## Yujun Zhang 
```
