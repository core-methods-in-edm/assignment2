...
```{r}
st <- EDGE %>% group_by(from) %>% summarise(count_new = sum(count))
names(st) <- c("id", "count")
tc <- EDGE %>% group_by(to) %>% summarise(count_new = sum(count))
names(tc) <- c("id", "count")
new <- full_join(st,tc)
g <- graph.data.frame(EDGE, directed=TRUE, vertices= new)
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count, vertex.size = new$count)
...
table <- select(hudk4050_classes, Q8:Q9, Q1:Q7)
table$Q9 <- paste(table$Q8, table$Q9, sep = " ")
table <- table[3:55, 2:7]
table <- table[-c(5,14,18,21,29,34,45),]
table$Q1 <- paste(table$Q1, sep = " ")
table_new <- gather(table, name, class, 2:6)
table_new <- table_new[,-2]
table_new <- cbind(table_new, 1)
names(table_new) <- c("name", "class", "num")
table_new <- table_new[-which(is.na(table_new$class)),]
pc <- spread(table_new, class, num)
pc[is.na(pc)] <- 0
pc <- pc[,-1]
rownames(pc) <- table$Q9
pc <- as.matrix(pc)
pp <- pc %*% t(pc)
diag(pp) <- NA
```
...
```{r}
library(igraph)
g <- graph.adjacency(pp, mode = "undirected")
plot(g, layout = layout.fruchterman.reingold)  

which(degree(g)==max(degree(g)))
# Yiwei Qi  23
which(betweenness(g)==max(betweenness(g))) 
# David Pearce   35
