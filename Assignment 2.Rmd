...
plot(g,layout=layout.fruchterman.reingold, vertex.color=V(g)$color, edge.width=EDGE$count,edge.lty=6,edge.arrow.size=0.3, vertex.size=V(g)$size, vertex.label.color="grey", vertex.frame.color=VERTEX$gender,vertex.shape=V(g)$shape, frame=F, main = "Network Plot of Teachers and Students", margin = 0.01)
...
#legend(x=1.5, y=0.1, c("Teacher","Student"), pch=21,col = "black", pt.cex=2,cex=.8, bty="n", ncol=1)
```
...
plot(person.g, layout=la, vertex.size=10,edge.width=0.5, vertex.label=V(person.g)$name)
...
plot(person2.g, layout=la, vertex.size=15,edge.width=e.wt, vertex.label=V(person2.g)$name)
...
```
#Failed Method 1
#classes <- classes_long %>% spread(Full.name, List)
#convert to binery
#classes[is.na(classes)] <- 0
#classes[,-1] <- apply(classes[,-1], c(1,2), function(x) ifelse(x != 0, 1, x))
#classes[,-1] <- sapply(classes[,-1 ], as.numeric)
#data <- as.matrix(classes)