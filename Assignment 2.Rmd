...
library(dplyr)
library(tidyr)
st_sum<-EDGE%>%group_by(to)%>%summarise(sum(count))
names(st_sum)<-c("id","count")
tc_sum<-EDGE%>%group_by(from)%>%summarise(sum(count))
names(tc_sum)<-c("id","count")
sum<-bind_rows(tc_sum,st_sum)
...
D1<-D1[-(1:2),]
D2<- select(D1, Q8, Q9, Q1, Q3, Q4, Q5, Q6)
D3<-tidyr::unite(D2,Name,'Q8','Q9',sep=" ",remove=TRUE)
D4<-tidyr::gather(D3,Course_Label, Course_Name,'Q1','Q3','Q4','Q5','Q6', na.rm = TRUE, convert = FALSE)
D4<-select(D4, Name, Course_Name)
D4<-filter(D4, Course_Name>0, Course_Name != "HUDK 4050")
D4$Count <- 1
D5 <- spread(D4, Course_Name, Count)
