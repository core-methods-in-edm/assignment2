...
#We have to get the packages igraph
...
HUDK_data<-read.csv("hudk4050-classes.csv")

#selcet varibles and clean spaces

Class_kind<- HUDK_data %>% select(id = 'First.Name',
                                class1 = 'Class.1',
                                class2 = 'Class.2',
                                class3 = 'Class.3',
                                class4 = 'Class.4',
...
Class_gather<- Class_kind %>%
                        gather(key = class,value = course_code,
...
Class_gather$course_code<- gsub(pattern = " ",
                                replacement = "",  
                                x=Class_gather$course_code)
...
Class_gather_matrix<-Class_gather %>% spread(key=course_code,
                                             value = enrolled,
                                             fill=0)
#Then we convert matrix

Class_gather_matrix2<-Class_gather_matrix %>% select(-id)
Class_gather_matrix3<-as.matrix(Class_gather_matrix2)

#Transpose

Class_gather_matrix4<-t(Class_gather_matrix2)
Class_gather_matrix5<-Class_gather_matrix3 %*% Class_gather_matrix4

#Rename

rownames(Class_gather_matrix5)<- Class_gather_matrix$id
colnames(Class_gather_matrix5)<- Class_gather_matrix$id

# Then aviod problem change to NA

diag(x=Class_gather_matrix5)<- NA

#Get a new graph data

Graph_data<- graph_from_adjacency_matrix(Class_gather_matrix5, mode = "undirected")

#Plog into data

plot.igraph(Graph_data, layout = layout.fruchterman.reingold, vertex.size = 10,
...
```{r}
#betweens centrality
betweeness_centrality<- betweenness(Graph_data)
