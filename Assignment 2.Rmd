...
plot(g2,layout=layout.fruchterman.reingold,vertex.color=VERTEX2$gender, edge.arrow.size=0.1, vertex.size=VERTEX2$count + 20)
...
which.max(degree(g3))
which.max(betweenness(g3))

#Note-taking version:
#import data
DC1 <- read.csv("hudk4050-classes.csv")
DC1
library(tidyr)
library(dplyr)
...
#combine classes into the same column
DC3 <- DC2 %>% gather(classnum, classcode, `Class.1`, `Class.2`, `Class.3`, `Class.4`, `Class.5`, `Class.6`) %>% select(-c(classnum))

#eleminate the space in class column
DC3$classcode = gsub(" ", "", DC3$classcode)
...
#spread the name according to count
DC4 <- DC3 %>% spread(classcode,Count)

#putting the first column into the rownmaes
row.names(DC4) = DC4$name
...
#people people matrix, same people = NA
diag(DC6) <- NA
...
#another method
plot(g3, layout=layout.fruchterman.reingold,edge.arrow.size=0.01)
