...
names(EDGE) <- c("from", "to", "count")#Name the column
...
```{r}
data_t<-EDGE%>%group_by(from)%>%
  summarise(count=sum(count))
names(data_t) <- c("id", "count")
...
```{r}
data_s<-EDGE%>%group_by(to)%>%
  summarise(count=sum(count))
names(data_s) <- c("id", "count")
print(data_s)

```
```
#combine data_t and data_s
```{r}
TOTAL = rbind(data_t,data_s)
```
```{r}
VERTEX1 <- TOTAL
summary(VERTEX1)
...
plot(g1,layout=layout.fruchterman.reingold, vertex.color=VERTEX1$id, vertex.size=VERTEX1$count)
```
