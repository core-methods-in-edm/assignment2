...
```{r}
stu <- EDGE %>% group_by(to) %>% summarize(sum(count))
tea <- EDGE %>% group_by(from)%>% summarize(sum(count))
names(stu) <- c("id", "counts")
names(tea) <- c("id", "counts")
VERTEX1 <- bind_rows(stu, tea)

g1 <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX1)
plot(g1,layout=layout.fruchterman.reingold, vertex.size = VERTEX1$counts)
...
class <- select(class, Q8, Q9, Q1, Q3, Q4, Q5, Q6, Q7)
...
class <- tidyr::gather(class, "order", "name",2:7)
class <- class[, -2]
class <- filter(class, !class$name == "")
class$name <- gsub(" |-", "", class$name)
class <- filter(class, !class$name == "HUDK4050")
class$count <- 1
class <- tidyr::spread(class, name, count, fill = 0)

pn <- class
rownames(pn) <- class$student
pn <- pn[,-1]
t1 <- t(pn)
t2 <- as.matrix(pn)
network <- t2 %*% t1
...
```{r}
library(igraph)
g2 <- graph_from_adjacency_matrix(network)
plot(g2,layout=layout.fruchterman.reingold, vertex.size = 4, vertex.label.dist = 0.4, vertex.label.color = "black", edge.arrow.size = 0.4, edge.arrow.width = 0.4)


centr_betw(g2, directed = TRUE, nobigint = TRUE, normalized = TRUE)
