...
#edge list from A to B;from A to c #vertex list, ABC
...
```{r}
#sizes the student vertices the number of disciplinary actions  recieved
EDGE
VERTEX_T<-EDGE %>%
  group_by(from) %>%
  summarize(sum(count))
...
names(VERTEX_T)<-c("id","count")
names(VERTEX_S)<-c("id","count")
VERTEX_total<-bind_rows(VERTEX_T,VERTEX_S)
VERTEX2<-merge(VERTEX,VERTEX_total,by="id")

g2<- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX2)
#sizes the teacher vertices the number of disciplinary actions  given
...
df<-df%>%
  ...
  gather(My_orders,My_courses,-Full.Name)%>%
  select(Full.Name,My_courses)

#Erase all the empty values by filtering 
df<-df%>%
  filter(My_courses!="")%>%
  arrange(Full.Name)
...
person_course<-spread(df,My_courses,Count,fill=0)
...
#Creating a matrix for person-person
name_course<-as.matrix(person_course)
course_name<-t(name_course)
person_person_matrix<-name_course%*%course_name

#Creating a grap from an adjacency matrix(person-person)
g3<-graph_from_adjacency_matrix(person_person_matrix,mode="directed",diag=FALSE)
plot(g3)

#Betweenness centrality
betweenness(g3,v=V(g3),directed=TRUE)

#find the max degree and the most
max(degree(g3))
V(g3)[degree(g3)==max(degree(g2))] 
# obviously LINGTONG_LI is the most central person in the network
