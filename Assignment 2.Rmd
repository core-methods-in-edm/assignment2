...
head(df)
str(df) # all variables are factors
df<-lapply(df,str_replace," ","")
df<-lapply(df,str_replace,"-","")
df<-lapply(df,toupper)
df<-as.data.frame(df)
gsub(pattern=".*001$",replacement = "here",)
df<-subset(df,)
...
data_clean <- function(arg) { 
  outcome<-arg
  output1<-gsub(pattern="\\s|-",replacement="",arg)
  if (output1!=arg){outcome<-output1}
  toupper(outcome)
}
...
df<-as.data.frame(df)
df_no_zimo<-dplyr::filter(df,df$First.Name!="ZIMO")

#df_no_zimo[14,4:5]<-c("YIXIAO","LI","HUDK4050","IFSF4090","EDPS4002","EDPS4021","","")
#as.factor(c("IFSF4090","EDPS4002","EDPS4021"))
#df_no_zimo[14,4:6]<-as.factor(c("IFSF4090","EDPS4002","EDPS4021"))
#df<-lapply(df, data_clean)

g_hudk4050<-graph.data.frame(,directed,)
