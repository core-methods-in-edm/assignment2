...
library(igraph)
D1 <- read.csv("discipline-data.csv", head = TRUE)
...
plot(g,layout=layout.fruchterman.reingold, vertex.size = VERTEX$count, edge.arrow.size = 0.5, vertex.color=VERTEX$gender, edge.width=EDGE$count)
...
D3 <- read.csv("HUDK4050-2018-classes.csv", header = FALSE)
D3 <- D3[-c(1,3),]
...
gg <- graph.adjacency(D6, mode = "undirected")
plot(gg, layout = layout.fruchterman.reingold())

bc <- betweenness(gg, v=V(graph), directed = FALSE, weights = NULL, nobigint = TRUE, normalized = FALSE)

dg <- degree(gg, v = V(graph), mode = c("all", "in", "total"), 
