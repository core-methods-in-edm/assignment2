...
STU.SUM <- EDGE%>%group_by(to)%>%summarise(sum(count)) 
TE.SUM <- EDGE%>%group_by(from)%>%summarise(sum(count)) 
names(STU.SUM) <- c("id","count")
names(TE.SUM) <- c("id","count")
TOTAL <- bind_rows(STU.SUM,TE.SUM)
...
## first, clean data - number and course descrip.
...
D6 <- select(D5,Name,Class.Num)
names(D6) <- c("from", "to")
D6 <- filter(D6,to>0,to!="HUDK4050")
D6$count <- 1
D6 <- unique(D6)
D7 <- spread(D6,to,count,fill=0)
D7 <- as.data.frame(D7)
Max1 <- as.matrix(D7)
row.names(Max1)<- D7$from
class(Max1) <- "numeric"
Max1 <- Max1[,-1]
Max2 <- t(Max1)
Max3 <- Max1 %*% Max2
diag(Max3)
g9 <- graph.adjacency(Max3,mode="undirected")
degree(g9)
betweenness(g9)
pos_max <- which.max(degree(g9))
print(pos_max)
pos_max2 <- which.max(betweenness(g9))
print(pos_max2)
```
#
Who is the most central person in the network?
LintongLi YujunZhang
#
```{r}
plot(g9,layout=layout.fruchterman.reingold,vertex.color="grey")
```
