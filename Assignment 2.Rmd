...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count,vertex.size=16)  
```
...
```{r}
D3 <- read.csv("hudk4050-classes.csv", header = TRUE, sep = ",")

#concatenating the names to create unique identifiers
D3$name <- do.call(paste, c(D3[c("First.name", "Last.name")], sep = " ")) 

#dropping the unnecessary columns
library(tidyr)
D4 <- dplyr::select(D3, 3:8)

#Reshaping the data
D5 <- tidyr::gather(D4, "name", "Course1", 1:5)

#renaming columns
names(D5) <- c("name", "course_number", "course_name")

#Dropping blanks
D6 <- dplyr::filter(D5, course_name > 0)

#Dropping supurfluous column
D6 <- dplyr::select(D6, 1,3)

#Adding an intersection column
D6$intersection <- "1"

#More reshaping to create grid of names by classes
D7 <- tidyr::spread(D6, "course_name", "intersection")

#replacing N/A with 0
D7[is.na(D7)] <- 0

#I am stuck on replicating this part of the Paul Revere code :-()
person.net <- D7 %*% t(D7)
