...
NS<-group_by(EDGE,to)%>%summarise(time=sum(count))

plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, vertex.size=NS$time, edge.width=EDGE$count)
...
class1<-filter(class,Progress==100)

class2<-select(class1, Q8,Q9,Q1,Q3,Q4,Q5,Q6,Q7)

class3<-gather(class2,Q,course,Q1,Q3,Q4,Q5,Q6,Q7)

class4<-unite(class3,name,"Q8","Q9")

class5<-count(class4,name,course)

class6<-filter(class5,class5$course!="")

class6$course<-gsub(" ","", class6$course)

class6<-filter(class6,class6$course!="HUDK4050")

class7<-spread(class6,course, n)

class7[is.na(class7)] <- 0 

class7<-as.data.frame(class7)

rownames(class7)<-class7$name
...
```{r}
g2 <- graph.adjacency(M3,mode="undirected") 
