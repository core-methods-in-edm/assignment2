...
H1$Last.name<-do.call(paste, c(H1[c("First.name", "Last.name")], sep = " "))
H2<-dplyr::select(H1,Last.name,Course1,Course2,Course3,Course4,Course5)
H5<- H2 %>% tidyr::gather(course,name,2:6)
```
Gather command just dont't work on H4 whywhywhy
#transpose the H2
H3<-t(H2)
#make the columns to be the name
colnames(H3)<-H3[1, ]
H4<-H3[-1, ]
#gather
H5<- H4 %>% tidyr::gather(course,name,1:28)

Sort out the edge
```{r}
H5<-dplyr::select(H5,Last.name,name)
#why did it become a string of the characters
#> H5$name <- ifelse(H5$name == "", NA, H5$name)
> View(H5)
> H5 <- na.omit(H5)
> View(H5)

H6<-ifelse(H5$name=="",NA,H5$name)
H6nona<-na.omit(H6)
EDGE2 <- dplyr::count(H5nona, Last.name, name)
