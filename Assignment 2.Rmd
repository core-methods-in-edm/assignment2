...
```{r}
D1<-read.csv("/Users/liuxiaojue/Documents/2019datamining/assignment2/discipline-data.csv")
...
EDGE.student<-EDGE%>%group_by(to)%>%summarize(sum(count))
names(EDGE.student)<-c("id","num_dis")
EDGE.student
EDGE.teacher<-EDGE%>%group_by(from)%>%summarize(sum(count))
names(EDGE.teacher)<-c("id","num_dis")
EDGE.teacher
VERTEX2 <- bind_rows(EDGE.student, EDGE.teacher)
VERTEX2
g2 <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX2)
g2

plot(g2,layout=layout.fruchterman.reingold, edge.width=VERTEX2$num_dis)
...
plot(h,layout=layout.fruchterman.reingold,vertex.size=5,
     vertex.label.dist=0.5, vertex.color="pink",vertex.label.color="black" ,edge.arrow.size=0.1)

betweenness.h<-betweenness(h)
betweenness.h
which(betweenness.h==max(betweenness.h))

degree.h<-degree(h)
degree.h
which(degree.h==max(degree.h))

# We can conclude that Yujun Zhang is the most central person in the class.
