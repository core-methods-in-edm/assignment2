...
g <- graph.data.frame(EDGE, directed = TRUE, vertices = VERTEX)
plot(g, layout = layout.fruchterman.reingold, vertex.color = VERTEX2$gender, edge.width = EDGE$count, vertex.size = VERTEX$count*2)
...
#Reshape data to create a "course" variable
D2 = tidyr::gather(D1, course.label, course, Q1, Q3, Q4, Q5, Q6, na.rm = TRUE, convert = FALSE)
...
fit = kmodes(D6, 3, iter.max = 10, weighted = F)
...
D7 = data.frame(D6, fit$cluster)


ggplot(data = D7, aes(x= D6$., y= D6$., color=as.factor(fit$cluster))) +
  geom_point(data = D7, aes(x= D6$., y= D6$., color=as.factor(fit$cluster)), size=3)
