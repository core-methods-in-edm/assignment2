...
dis_t <- count(D2, tid)
dis_s <- count(D2, stid)
names(dis_t) <- c("id", "num_actions")
names(dis_s) <- c("id", "num_actions")
dis_total <- bind_rows(dis_s, dis_t)
VERTEX$id <- as.factor(VERTEX$id)
VERTEX_new <- merge(VERTEX, dis_total, by = "id")

g1 <- graph.data.frame(EDGE, directed = TRUE, vertices = VERTEX_new)
plot(g1, layout = layout.fruchterman.reingold, vertex.size = VERTEX_new$num_actions)
```
...
library(tidyr)
library(dplyr)
D3 <- read.csv("hudk4050-classes.csv", header = TRUE, na.strings = "")

# remove space from the name of the classes
D4 <- data.frame(apply(D3, 2, function(x)gsub('\\s+', '',x)))

# get full name
D4 <- unite(D4, "Name", c("First.Name", "Last.Name"), sep = " ")

# create a person-class matrix
D5 <- gather(D4, "Class", "Class_name", -1, na.rm = TRUE)
D5$count <- 1
D6<- pivot_wider(D5, id_cols = "Name", names_from = "Class_name", values_from = "count")
D6[is.na(D6)] <- 0
M1 <- as.matrix(D6[,-1])
rownames(M1) <- D6$Name
...
# plot the matrix
library(igraph)
g3 <- graph_from_adjacency_matrix(M2, mode = "undirected", weighted = NULL, diag = FALSE)
plot(g3, layout = layout.fruchterman.reingold)

# generate betweeness centrality and dregree
betweenness(g3, directed = FALSE, weights = NULL, normalized = FALSE)
centr_degree(g3)

# The most central person in the network is XUDIAN ZHANG.
