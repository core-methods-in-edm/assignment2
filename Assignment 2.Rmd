...
```{r}
EDGE.st <- EDGE %>% group_by(to) %>% summarize(sum(count))
names(EDGE.st) <- c("id", "count")
EDGE.tc <- EDGE %>% group_by(from) %>% summarise(sum(count))
EDGE.tc
names(EDGE.tc) <- c("id", "count")
total <- bind_rows(EDGE.st, EDGE.tc)
...
library(dplyr)
library(tidyr)
DC1 <- read.csv("hudk4050-classes.csv", header=TRUE)
