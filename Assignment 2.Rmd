...
```{r}
EDGE <- count(D2, tid, stid)
EDGE.st <- EDGE%>%group_by(stid)%>%summarise(sum(n))
EDGE.t <- EDGE%>%group_by(tid)%>%summarise(sum(n))
names(EDGE.st) <- c("id", "count")
names(EDGE.t) <- c("id", "count")
EDGE.total <- bind_rows(EDGE.st,EDGE.t)
VERTEX2 <- full_join(VERTEX,EDGE.total,by="id")
g2 <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX2)
plot(g2,layout=layout.fruchterman.reingold, vertex.color=VERTEX2$gender, vertex.size=EDGE.total$count)
...
plot(h,layout=layout.fruchterman.reingold,vertex.size=2,
     vertex.label.dist=1, vertex.color="red", edge.arrow.size=0.3)

###By betweenness, Yujun Zhang is the most central person in this network; by degree, Lintong Li is the most central.
betweenness(h)
degree(h)
```
