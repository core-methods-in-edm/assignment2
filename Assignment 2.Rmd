...
library(dplyr)
#students:
student1 <- count(D2,stid)
names(student1) <- c("id","count")
#teachers:
teacher1 <- count(D2,tid)
names(teacher1) <- c("id","count")
binding <- bind_rows(teacher1, student1)
G <- graph.data.frame(EDGE, directed = TRUE, vertices = binding)
plot(G,layout=layout.fruchterman.reingold)
binding$id <- as.factor(binding$id)
plot(G, layout=layout.fruchterman.reingold,vertex.color=binding$id)
...
```{r}
library(tidyr)
DF <- read.csv("HUDK4050_2018_classes.cvs")
DF1 <- select(DF, Q8:Q6)
DF2 <- unite(DF1,"Name",'Q8','Q9', sep= " ")
DF2$Name <- as.factor(DF2$Name)
DF3 <- gather(DF2, "label", "course", Q1, Q3, Q4, Q5, Q6)
DF3 <- select(DF3, "Name","course")
DF3$course <- gsub(" ","", DF3$course)
DF3 <- mutate(DF3, count = 1)
DF4 <- spread(DF3, course, count)
DF4<- ifelse(is.na(DF4),0,1)
DF5 <- DF4 %*% t(DF4)
diag(DF5) <- 0
install.packages("igraph")
library("igraph")
g <- graph_from_adjacency_matrix(DF5, mode = "undirected", weighted = TRUE)
plot(g, layout=layout.fruchterman.reingold)
```
```
