...
n.st <- unique(edges.st)
edges.tch <- EDGE %>% group_by(from) %>% summarize(sum(count))
n.te <- unique(edges.tch)
names(edges.st) <- c("id", "count")
names(edges.tch) <- c("id", "count")
total <- rbind(edges.st, edges.tch)
...
plot(g2,layout=layout.fruchterman.reingold, vertex.color=VERTEX2$gender, vertex.size=VERTEX2$count+17, edge.arrow.size = 0.5)
...
DT6 = DT5 %*% t(DT5)
diag(DT6) <- NA
g3 = graph.adjacency(DT6, mode = "undirected")
