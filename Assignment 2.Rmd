...
vert_color <- ifelse(VERTEX$group == "teacher","springgreen","slateblue")
plot(g,edge.arrow.size=.3, layout=layout.fruchterman.reingold, vertex.color=vert_color, vertex.size= 2 * degree(g))
...
library(tidyverse)
library(igraph)
MX1 <- read.csv("hudk4050-classes.csv", header=TRUE, as.is = TRUE)
MX1 <- unite(MX1,"name",Last.Name,First.Name, sep = ", ")
MX1 <- MX1 %>% mutate_all(funs(gsub(".*4050","",.))) %>%
  mutate_all(funs(gsub("\\s+","",.))) %>%
  gather("class_number","course_num",2:7) %>%
  select(-class_number)

MX1 <- MX1[!(MX1$course_num=="" | MX1$course_num==" "),] 

MX1$count <- c(1)
MX2 <- MX1 %>% spread(course_num,count,fill="0") %>%
...
g2 <- graph.adjacency(MX3,mode="undirected")
