...
DF_class <- select(class_2018,Q8,Q9,Q1:Q6)
DF_class <- DF_class[-c(1,2,3,39),]
names(DF_class) <- c("first_name","last_name","c1","c2","c3","c4","c5")
DF_class <- unite(DF_class,"name","first_name","last_name",sep=" ",remove=TRUE)
List <- gather(DF_class,Class_Label,Class_Name,2:6)
List <- select(List,name,Class_Name)
List$count <- 1
List <- filter(List,Class_Name != "")
Detail <- spread(List, Class_Name, count)
...
g <- graph_from_adjacency_matrix(person_person_matrix2, mode = c("undirected"), weighted = TRUE, diag = FALSE,add.colnames = NULL, add.rownames = NA)
