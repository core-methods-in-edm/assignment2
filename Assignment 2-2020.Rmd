...
DF3 <- DF1 %>% group_by(lesson_name, hash) %>% summarise(att=sum(attempt))
...
DF3 <- DF1 %>% group_by(lesson_name,hash) %>% summarise(att=sum(attempt))
DF3 <- DF3 %>% group_by(lesson_name) %>% summarise(correct=mean(att))
...
```{r}
DF4$correct <- ifelse(DF4$DF1.correct=="TRUE", 1, 0) #treat NAs as zeros because no answer has no credit (just created that rule!)
...
```{r}
DF5 <- DF4 %>% group_by(DF1.hash, DF1.lesson_name) %>% summarise(score=sum(correct))
```
