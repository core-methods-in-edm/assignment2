...
```{r}
score = round(rnorm(200, 75, 15))
score = data.frame(score)
score = filter(score, score <= 100)
score = slice(score, 1:100)

interest = sample(c("sport", "music", "nature", "literature"), 100, replace = TRUE)
interest = data.frame(interest)

stid = seq(1:100)
stid = data.frame(stid)

data = cbind(stid, score, interest)
```
...
```{r}
label = letters[1:10]
data$breaks<-cut(data$score, breaks = 10, labels = label)
```

4. Now using the colorbrewer package
(RColorBrewer; http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3)
design a palette and assign it to the groups in your data on the histogram.
...
data$colors = brewer.pal(10, "Paired")
# Use named palette in histogram
hist(data$score, breaks = 10, col = data$colors)
```

5. Create a boxplot that visualizes the scores for each interest group and color each interest group a different color.

```{r}
interest.col = brewer.pal(4,"Paired")
boxplot(score ~ interest, data, col = interest.col)
```
...
```{r}
data$login = sample(1:25, 100, replace = TRUE)
```

7. Plot the relationships between logins and scores. Give the plot a title and color the dots according to interest group.

```{r}
data$col1 = ifelse(data$interest == "music", "red",
                   ifelse(data$interest == "literature", "green",
                          ifelse(data$interest == "sport", "pink", "blue")))
plot(data$login, data$score, col = data$col1, main = "Logins vs. Scores", xlab = "Logins", ylab = "Scores")
```
...
```{r}
DF3 = DF2 %>% group_by(hash, lesson_name) %>% summarise(sum = sum(attempt, na.rm = TRUE))
```
...
```{r}
DF3 = DF3 %>% spread(lesson_name, sum)
```
...
```{r}
DF5 = DF4 %>% group_by(hash, lesson_name) %>% summarise(mean = mean(correct, na.rm = TRUE))
```
...
Finally use the knit function to generate an html document from your work. Commit, Push and Pull Request your work back to the main branch of the repository. Make sure you include both the .Rmd file and the .html file. 
