...
S3$colors <- brewer.pal(10, "Paired")
...
# According to the relationships shown in the paired graph, the relatioship between Petal.Length and Petal.Width might be appropraiet to run a correlation on.
```
...
```{r}
DF3 <- DF2 %>% group_by(hash, lesson_name) %>% summarise(sums = sum(attempt, na.rm = T), .groups = "keep")
...
```{r}
DF3 <- DF3 %>% spread(key = lesson_name, value = sums)
head(DF3)
...
```{r}
DF5 <- DF4 %>% group_by(hash, lesson_name) %>% summarise(mean_score = mean(correct, na.rm = T), .groups = "keep")
```
...
DF6$datetime <- as.POSIXlt(DF6$datetime,tz = "UTC", origin = "1968-01-01")
DF6$datetime <- format(DF6$datetime, format = "%m-%d-%Y")
DF6$correct <- ifelse(DF6$correct == "TRUE", 1, 0)
DF6 <- DF6 %>% group_by(datetime) %>% summarise (daily_average = mean(correct, na.rm = T), .groups = "keep")
