...
set.seed(1)
df_interest <- data.frame(scores, interest)
...
```{r}
#Generate a histogram of the watch time for the year 2018
hist(df_interest$scores)

#Change the number of breaks to 5
hist(df_interest$scores, breaks = 5)

hist(df_interest$scores, breaks = c(30,60,70,80,90,100))
...
df_interest$grade <- cut(df_interest$scores, breaks = c(30,60,70,80,90,100), labels = letters[5:1])
...
hist(df_interest$score,breaks = c(30,60,70,80,90,100), col=brewer.pal(7,"Blues") )
...
boxplot(df_interest$score~df_interest$interest,data=df_interest,col=brewer.pal(4,"Pastel1"))
```
...
```{r}
set.seed(1)
df_interest$login <- sample(1:25, 100, replace = TRUE)
```

7. Plot the relationships between logins and scores. Give the plot a title and color the dots according to interest group.

```{r}
plot(df_interest$login, df_interest$scores, col=factor(df_interest$interest), xlab = "Number of logins", ylab = "Scores", main = "Relationships between logins and scores")
legend("bottomleft", legend=levels(factor(df_interest$interest)), text.col=seq_along(levels(factor(df_interest$interest))), cex=0.5)
...
```{r}
plot(iris)
##Relationship between Petal.Width and Petal.Length is correlated.
```
...
```{r}
DF3 <- DF2 %>% group_by(lesson_name, hash) %>% 
  summarise(Total = sum(attempt, na.rm = TRUE))
```
...
```{r}
library(reshape2)
DF3<-dcast(DF3, hash ~ lesson_name)
```
...
```{r}
DF5 <- DF1[,!names(DF1) %in% "correct"]
DF5$correct <- DF4$correct
DF5 <- DF5 %>% filter(!is.na(correct))
DF5 <- DF5 %>% group_by(hash,course_name) %>% summarise(meanscore=mean(correct))
```
...
```{r}
DF6 <- DF1[,!names(DF1) %in% "correct"]
DF6$correct <- DF4$correct
DF6 <- DF6 %>% filter(!is.na(correct))
DF6$datetime <- as.POSIXct(as.numeric(DF6$datetime),tz='UTC',origin='1970-1-1')
DF6$datetime <- factor(as.Date(DF6$datetime))
DF6 <- DF6 %>% group_by(datetime) %>% summarise(meanscore=mean(correct))
```
