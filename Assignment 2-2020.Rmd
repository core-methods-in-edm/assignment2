...
# try to bind S1 and S2
# Create a new data frame

S2 <- data.frame(rep(100, 100-length(S1$score)))

# change the name of the row
names(S2) <- "score"

#bind two data frames and make sure the names of header matches and same data type
S3 <- bind_rows(S1, S2)

#Alternative 2
# More often use this one, use ifelse statement
# if S3$score >= 100, make it 100, otherwise, make it S3$score
...
#label them in different letter, need 20 letters for 20 breaks
label <- letters[1:10]

#cut score into 20 pieces and label them in leable
...
```{r}
plot(S3$login, S3$score, col=S3$colors, main = "Compare Student Logins and Scores")
plot(S3$login, S3$score, col=interest.col, main = "Compare Student Logins and Scores")
