...
D3 <- D1 %>% group_by(year) %>% 
  summarise(mean_key = mean(key.points))
...
score <- rnorm(100,75,20)
hist (score, breaks=30)
S1 <- data.frame(score)
library(dplyr)
S1 <- filter(S1,score<=100)
hist(S1$score)
S2 <- data.frame(rep(100,11))
names(S2) <- "score"
S3 <- bind_rows(S1,S2)
...
```{r}
DF4$correct <- ifelse(DF4$correct==TRUE,1,ifelse(is.na(DF4$correct)==TRUE,NA,0))
```
