...
dat<- filter(dat, Score <= 100)
#round rounds numbers to whole number values
dat <- round(dat)
#sample draws a random samples from the groups vector according to a uniform distribution
dat2 <- data.frame(round(runif(n = 5,min = 50, max = 100)))
names(dat2) <- "Score"
...
```{r}
hist(dat3$Score, breaks = 5, main = "Histogram of Score")
```
...
label <- letters[1:10]
dat3$breaks <- cut(dat3$Score, breaks = 10, labels = label )
...
dat3$colors <- brewer.pal(5, "Set3") 
#Use named palette in histogram
hist(dat3$Score, col = dat3$colors, main = "Histogram of Score" )
```
...
interest <- c("sport", "music","nature","books")
dat3$interest <- sample(interest, 100, replace = T)
interest.col <- brewer.pal(4, "Set1")
boxplot(Score~interest, dat3, col = interest.col )
```
...
```{r}
DF3 <- DF2 %>% group_by(hash, lesson_name) %>% summarise(all_attempts = sum(attempt))
head(DF3)
```
...
```{r}
library(reshape)
DFF <- cast(DF3, hash~lesson_name, value ="all_attempts" )
head(DFF)
```
...
DF4$correct<- as.integer(DF4$correct)
head(DF4)
```
...
```{r}
DF5 <- DF4 %>% group_by(hash, lesson_name)%>% summarise(meanScore = mean(correct, na.rm = T))
head(DF5)
```
...
```{r}
DF6 <- DF1[,c("datetime","correct")]
DF6$correct <- ifelse(DF6$correct == TRUE, 1, 0)

class(DF6$datetime) = c ("POSIXct") 

DF6$datetime <- format(DF6$datetime, format = "%m-%d-%Y")
DF6 <- DF6 %>% group_by(datetime) %>% summarise(average_score =mean(correct,na.rm = TRUE))
DF6
```
