...
sc <- rnorm(100,75,15)
sc <- ifelse(sc>=100,100,sc)
sc <- round(sc,0)
df <- data.frame(sc)


interest_groups <- c("sport","music","nature","literature")
interest <- sample(interest_groups, 100, replace=TRUE)
...
labels = seq(1,10,1)
df$score_group <- cut(df$sc, breaks = 10,labels = labels)
df$score_group
```
...
boxplot(sc ~ interest,df,col = interest.col)
...
```{r}
plot(df$login, df$sc, col = interest.col, main="student login vs. student score")
```
...
pairs(iris)
```
Petal.Length vs Petal.Width is appropriate to run a correlation on. 
