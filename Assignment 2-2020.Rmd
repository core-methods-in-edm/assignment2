...
interst.col <- brewer.pal(4, "Dark2")

boxplot(score ~ interest, S3, col = interst.col)
```
...
```{r}
DF1 <- read.csv(file = 'swirl-data.csv')
```
...
```{r}
group_by(DF2, lesson_name)
DF3 <- summarise(DF2, sum_attempts = sum(DF2$attemps))
```
