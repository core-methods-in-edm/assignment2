...
S2<-data.frame(rep(100,100-NROW(S1)))#in S1, the total number is 95 instead of 100, so we add five 100 manually. However, it means it is not randomly.
names(S2) <- "score"
S3<-bind_rows(S1,S2)#stick two command together


#alternative 2
#S1$score=ifelse(S1$score>=100,100,S1$score)#if else S3$score>=100,then make it 100, otherwise make it S3$score
#S1$score=round(S1$score,0)
interest = c("sport","music","nature","literature")
S3$interest=sample(interest, 100,replace=TRUE)
S3$stid=seq(1,100,1)

#alternative 3 - take the whole thing over--it will change the mean and sd
...
label=letters[1:10]#if want the letters capitalized, use LETTERS
...
S3$colors=brewer.pal(10,"Set3")#the first 10 colors in set3 
...
Iris
pairs(Iris)
#I think Petal length by petal width is appropriate to run a correlation on because you can see a clear linear relationship between them. 
```
I think Petal length by petal width is appropriate to run a correlation on because you can see a clear linear relationship between them.
...
```{r}
DF3 <- DF2 %>% group_by(hash,lesson_name) %>% summarise(sumattempt = sum(attempt))
```
...
```{r}
DF3.1=spread(DF3,key=lesson_name,value=sumattempt,drop=FALSE,convert=TRUE)
```
...
```{r}
DF5=data.frame(DF1[,c("hash","course_name","correct")])
DF5$correct=ifelse(DF5$correct=="TRUE",1,0)
DF5$correct=as.numeric(DF5$correct)
DF5 <- DF5 %>% group_by(hash,course_name) %>% summarise(meanscore = mean(correct,na.rm=TRUE))
```
...
DF6$datetime=format(as.Date(DF6$datetime,origin="1970-01-01"),"%m/%d/%y")
DF6$correct=ifelse(DF6$correct=="TRUE",1,0)
DF6$correct=as.numeric(DF6$correct)
DF6 <- DF6 %>% group_by(datetime) %>% summarise(avgcorrect = mean(correct))
```
