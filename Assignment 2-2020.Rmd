...
data = round(rnorm(500, 75, 15));
data = data.frame(data)
data = filter(data, data<=100)
data = slice(data,1:100)

data_score = sample(c("sport", "music", "nature", "literature"),100, replace = TRUE)
data_score = data.frame(data_score)

stid = seq(1:100)
stid = data.frame(stid)

data_final = cbind(stid, data, data_score)
...
```{r}
hist(data_final$data, breaks = 10)
...
label <- letters[1:10]
data_final$breaks <- cut(data_final$data, breaks = 10, labels = label)
...
data_final$colors <- brewer.pal(10, "Set3")
hist(data_final$data,breaks = 10,col = data_final$colors)
```
...
data_score.col <- brewer.pal(4, "Paired")
boxplot(data~data_score, data_final, col = data_score.col)
...
```{r}
AirPassengers <- data_frame(AirPassengers)
plot(AirPassengers)
```
...
```{r}
DF2 <- data_frame(DF1[,c("hash","lesson_name","attempt")])
```
