...
#Create a data frame of the average total key points for each year and plot the two against each other as a line

D3 <- D1 %>% group_by(year) %>% summarize(mean_key = mean(key.points))
...
#Alt 2
S1$score <- ifelse(score > 100, 100, S1$score) %>% round(0)

interest <- c("sport", "music", "nature", "literature")

S1$interest <- as.factor(sample(interest,100,replace = TRUE))
...
```{r}
hist(S1$score, breaks = 10, main = "Histogram of Student Scores", xlab = "Score", ylab = "Number of Students")
```
...
hist(S1$score, col = S1$colors, main = "Histogram of Student Scores", xlab = "Score", ylab = "Number of Students")
```
...
boxplot(score ~ interest, S1, col = interest.col, main = "Score Distribution by Student Interest Group")
```
...
plot(S1$login, S1$score, col = S1$interest,main = "Score vs. Login (Colored by Interest)",xlab = "Number of Logins",ylab = "Score")
legend(x="topright", legend = levels(S1$interest), col=1:4,pch=1, inset=c(-0.2,0))
```
...
pairs(iris)
```
#### It is appopriate to run correlation on all the continuous variables: Petal.Width, Petal.Length, Sepal.Width, and Sepal.Length
...
```{r}
DF3 <- DF2 %>% group_by(hash,lesson_name) %>% summarize(total_attempts = sum(attempt))
DF3 %>% head %>% formattable
```
...
```{r}
#removed lesson_names that were assumed to be errors
DF3 <- DF3 %>% spread(lesson_name, total_attempts) %>% select(-V1,-Fu,-`Grouping and Chaining w`,`Grouping and C`,-Lo,-Manipulatin,-Subsetti,-`Tidying Data with tid`,-Hierarchica,-Looking)
DF3 %>% head %>% formattable
```
...
```{r}
#removed rows that have a faulty lesson name
DF4 <- DF1 %>% select(hash, lesson_name, correct) %>% filter(!lesson_name %in% c("V1","Fu","Grouping and C","Grouping and Chaining w","Lo","Manipulatin","Subsetti","Tidying Data with tid","","-Hierarchica","-Looking"))
DF4 %>% head %>% formattable
```


8. Convert the `correct` variable so that `TRUE` is coded as the **number** `1` and `FALSE` is coded as `0`  
```{r}
DF4 <- DF4 %>% mutate(correct = ifelse(correct == "TRUE", 1, 0))
DF4 %>% head %>% formattable
```
...
```{r}
DF6 <- DF1 %>% filter(!is.na(datetime)) %>% mutate(correct = ifelse(correct == "TRUE",1,0),datetime = as.Date(as.POSIXct(datetime, origin = "1973-01-01"),format = "%m-%d-%y")) %>% group_by(hash, datetime) %>% summarize(total_correct = sum(correct, na.rm=T)) %>% ungroup() %>% group_by(datetime) %>% summarize(avg_correct = round(mean(total_correct),2)) %>% filter(datetime>"2019-12-31")
DF6 %>% formattable
```

#### Dates before 2020 were erased because those were assumed to be errors in the data. Dates before the course started may be explained by testing of the auto-grader. This also could be explained by students that took another course over the summer that used this grader.
