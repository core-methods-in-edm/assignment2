...
dt1 <- data.frame(score)
hist(dt1$score)
#dt1 <- filter(dt1, score <= 100)
#dt2 <- data.frame(rep(100,))
#names(dt2) <- "score"
#dt3 <- bind_rows(dt1,dt2)
dt3 <- dt1
dt3$score <- ifelse(dt3$score>100,100,dt3$score)
hist(dt3$score)

interest <- c("sport", "music","nature","literature")
dt3$interest <- sample(interest, 100, replace = TRUE)
dt3$stid <- seq(1,100,1)
...
label <- letters[1:10]
dt3$breaks <- cut(dt3$score, breaks = 10, labels = label)
...
dt3$colors <- brewer.pal(10,"Set3")
#Use named palette in histogram
hist(dt3$score,col = dt3$colors)
```
...
boxplot(score~interest, dt3,col = interest.col)
...
```{r}
dt3$login <- sample(1:25, 100, replace = TRUE)
...
```{r}
plot(dt3$score,dt3$login, type = "p", main = "Relationship between Login and Scores",col = interest.col)
...
```{r}
DF10 <- DF2 %>% group_by(lesson_name) %>% summarise(attempt = sum(attempt))
DF11 <- DF2 %>% group_by(hash) %>% summarise(attempt = sum(attempt))
lesson_name <-as.matrix(DF10$lesson_name)
hash_id <- as.matrix(DF11$hash)
DF12 <- matrix(0,nrow = nrow(hash_id),ncol =length(lesson_name))
colnames(DF12) <- lesson_name
rownames(DF12) <- hash_id
#DF12 <- DF12[,-1]

DF13 <- DF2 %>% group_by(hash,lesson_name) %>% summarise(attempt = sum(attempt))

for (i in 1:nrow(DF13)){
  for (j in 1:nrow(DF12)){
    if (DF13$hash[i] == rownames(DF12)[j]){
      for (k in 1:ncol(DF12)){
        if (DF13$lesson_name[i]==colnames(DF12)[k]){
          DF12[j,k] <- DF13$attempt[i]
        }
      }
...
```{r}
DF5 <- select(DF1, course_name, hash, correct)
DF5$correct <- ifelse(DF5$correct== "TRUE", 1, 0)
DF5 <- drop_na(DF5)

DF6 <- DF5 %>% group_by(hash, course_name) %>% summarise(total_question = n())

DF7 <- DF5 %>% group_by(hash, course_name, correct ==1) %>% summarise(correct = n())
DF7 <- filter(DF7,`correct == 1` == TRUE)

#Name the DF back to DF5 as required
DF5 <-merge(DF6, DF7,by = c("hash","course_name"))
DF5$mean = DF5$correct/DF5$total_question
DF5 <- select(DF5, -c("correct == 1"))
DF5
```
