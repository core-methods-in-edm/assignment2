...
S2 <- data.frame("score"=rep(100,100-NROW(S1)))
S3 <- bind_rows(S1,S2)
...
plot(S3$login, S3$score, col=S3$col1, main= "Student logins vs. Scores")
```
...
IR <- data.frame(iris)
plot(iris)
#It is appropriate to run a correlation on the relationship between Petal.length and Petal.Width.
```
...
```{r}
SD <- read.csv("swirl-data.csv", header = TRUE)
```
...
```{r}
DF1 <- data.frame("course_name" = SD$course_name, "lesson_name" = SD$lesson_name, "question_number"=SD$question_number, "correct"=SD$correct, "attempt"=SD$attempt, "skipped" =SD$skipped, "datetime" = SD$datetime, "hash"=SD$hash)
```

3. Create a new data frame that only includes the variables `hash`, `lesson_name` and `attempt` called `DF2`
```{r}
DF2 <- data.frame("hash"=SD$hash, "lesson_name" = SD$lesson_name, "attempt"=SD$attempt)
```
...
```{r}
DF3 <- DF1 %>% group_by(hash, lesson_name) %>% summarise(sum_attempts = sum(attempt)) 
```
...
```{r}
spread(DF3,lesson_name,sum_attempts)
```

7. Create a new data frame from `DF1` called `DF4` that only includes the variables `hash`, `lesson_name` and `correct`
```{r}
DF4 <- data.frame("hash"=DF1$hash, "lesson_name" = DF1$lesson_name, "correct"=DF1$correct)
```

8. Convert the `correct` variable so that `TRUE` is coded as the **number** `1` and `FALSE` is coded as `0`  
```{r}
DF4$recode_correct <- ifelse(DF4$correct == "TRUE", 1, 0)
```

9. Create a new data frame called `DF5` that provides a mean score for each student on each course
```{r}
DF1$recode_correct <- ifelse(DF1$correct == "TRUE", 1, 0)
DF5 <- bind_rows(filter(DF1, recode_correct== 1), filter(DF1, recode_correct== 0))
DF5 <- DF5 %>% group_by(hash,course_name) %>% summarise(mean_correct=mean(recode_correct))
```
...
```{r}
DF1$recode_datetime <- DF1$datetime
class(DF1$recode_datetime)=c("POSIXt","POSIXct")
DF1$recode_datetime <- format(DF1$recode_datetime,"%m/%d/%y")

DF <- data.frame("correct"=DF4$recode_correct, "date"= DF1$recode_datetime)
DF <- bind_rows(filter(DF, correct== 1), filter(DF, correct== 0))
DF6 <- DF %>% group_by(date) %>% summarise(mean_correct = mean(correct))
DF6 <- slice(DF6,3:36)
```
