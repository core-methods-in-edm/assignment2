...
```{r}
DF5 <-DF4 %>%  group_by(hash,lesson_name) %>% drop_na(correct)%>%  summarise(total_score=sum(correct), .groups='drop') %>% pivot_wider(names_from = lesson_name, values_from=total_score)
DF5
```
