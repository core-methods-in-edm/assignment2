...
label<-letters[1:10]
#letters is an inbuild function 如果想要大写的话，可以写LETTERS
...
#brewer.pal create a new variable storing these 10 colors and add it to your dataframe 
...
```{r}
s1$logins<-sample(1:25,100,replace=T)
```
...
```{r}
library(dplyr)
DF3 <- spread(DF3,lesson_name,sum)
DF3
```
...
DF6$datetime <- as.Date.POSIXct(DF6$datetime,tz = "UTC", origin = "2000-01-01")
DF6$correct <- ifelse(DF6$correct == "TRUE", 1, 0)
DF6 <- DF6 %>% group_by(datetime) %>% summarize(avg_correct = mean(correct))
```
