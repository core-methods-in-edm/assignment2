...
```{r}
hist(DF$scores, breaks = c(0,60,70,80,90,100), freq = TRUE)
```
...
grade <- cut(scores, c(0,60,70,80,90,100), labels = letters[1:5])
DF <- data.frame(stid,scores,grade,group)
```
...
display.brewer.pal(n=5, name="Pastel2")
#Use named palette in histogram
hist(DF$scores, breaks = c(0,60,70,80,90,100), freq = TRUE, col = brewer.pal(n=5, name = "Pastel2"))
```
...
boxplot(DF$scores~DF$group, col = brewer.pal(n=4, name = "Pastel2"),xlab = "Interest Groups", ylab = "Scores")
```
...
```{r}
login <- sample(1:25, 100, replace = TRUE)
DF <- data.frame(stid, scores, grade, group, login)
```

7. Plot the relationships between logins and scores. Give the plot a title and color the dots according to interest group.

```{r}
plot(DF$login, DF$scores, xlab = "Number of Logins", ylab = "Scores")
...
```{r}
DF2 <- DF1[, c("lesson_name", "hash", "attempt")]
```
...
```{r}
DF1$correct <- as.numeric(DF1$correct)
DF5 <- DF1 %>% group_by(course_name, hash) %>% summarise(meanscore = mean(correct))
...
```{r}
DF1$datetime <- as.Date(as.POSIXct(as.numeric(DF1$datetime),origin='1970-1-1'))
DF6 <- DF1 %>% group_by(DF1$datetime) %>% summarise(average=mean(correct)) 
```
