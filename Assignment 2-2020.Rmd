...
D3 <- D1 %>% group_by(year) %>% summarise(mean_key = mean(key.points,na.rm = TRUE))
...
score <- ifelse(scores>=100,100,scores)
classification <- c("sport","music","nature","literature")
interest <- sample(classification,100,replace = T)
stid <- seq(1,100,1)
Data_set <- data.frame(stid,score,interest)
```
...
label <- letters[1:10]
Data_set$breaks<-cut(Data_set$score,breaks = 10,labels = label)
...
Data_set$colors <- brewer.pal(10,"Paired")
#Use named palette in histogram
hist(Data_set$score,breaks=10,col = Data_set$colors)
...
boxplot(score ~ interest, Data_set, col=interest.col)
```
...
```{r}
Data_set$login <- sample(1:25,100,replace = T)
```

7. Plot the relationships between logins and scores. Give the plot a title and color the dots according to interest group.

```{r}
Data_set$col1 = ifelse(Data_set$interest=="music","red",ifelse(Data_set$interest=="literature","green",ifelse(Data_set$interest=="sport", "pink","blue")))
plot(Data_set$login, Data_set$score, main = "Student Logins vs. Scores", xlab = "Logins", ylab = "Scores", col=Data_set$col1)
...
```{r}
pairs(iris)
#Sepal.Length and Petal.Length, Pwtal.Length and Petal.Width, Sepal.Length and Petal.Width
```
