...
D4 <- droplevels(D4) # possible values 
...
```{r}
#creates a random sample with a mean of 75 and standard deviation of 20
#pmax sets a maximum value, pmin sets a minimum value
#round rounds numbers to whole number values
#sample draws a random samples from the groups vector according to a uniform distribution
s_size = 100
s_mean = 75
interests <- c("sport","music","nature","literature")

student_scores <- round(rnorm(s_size, s_mean, 20))

df <- data.frame(score = student_scores)
df <- filter(df, between(score, 0, 100))
# filter(df$, <=100)
#filter only works for df 
student_scores = sample(df$score, s_size, replace = TRUE)

student_interests <- sample(interests, s_size, replace = TRUE)
df <- data.frame(score = student_scores, interest = student_interests)
...
```{r}
hist(df$score, breaks = 10, xlab = "Scores", main = "The Distribution of Scores")
```
...
```{r}
label <- letters[1:10]
score_group <- cut(df$score, breaks = 10, labels = label)
...
hist_brewer_color <- brewer.pal(n = 8, name = 'BuPu')
hist(df$score, breaks = 10, col = hist_brewer_color, xlab = "Scores", main = "The Distribution of Scores")
```
...
```{r}
box_brewer_color <- brewer.pal(n=4,name = "Set3")
boxplot(df$score ~ df$interest, xlab = "interests", ylab = "scores",las = 1, main = "Boxplot by interests", col = box_brewer_color)
...
login_mean = (1+25)/2
login_sd = 25/3
logins <- round(rnorm(25,login_mean,login_sd),0)
logins <- filter(data.frame(logins),between(logins,1,25))
logins <- sample(logins$logins, 25 ,replace = TRUE)
logins
```
...
box_brewer_color <- brewer.pal(n=4,name = "Set3")
df <- cbind(df,logins)
plot(df$logins ~ df$score, col = box_brewer_color, main = "the relationships between logins and scores", pch = 19, xlab = "scores", ylab = "logins", las = 1)
```
...
```{r}
library(plyr)
DF4$correct <- revalue(DF4$correct, c("TRUE"=1))
DF4$correct <- revalue(DF4$correct, c("FALSE"=0))
```
...
DF5 <- DF4 %>%
  group_by(hash,lesson_name) %>%
  summarise(mean_score = mean(as.numeric(correct)))
