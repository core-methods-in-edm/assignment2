...
```{r}
plot(S3$login, S3$score, col=S3$colors, main = "Studet Logins vs Scores")
...
```{r}
DF5<-data.frame(DF1[,c("hash","lesson_name","correct")])
DF5$correct <- ifelse(DF5$correct == 'TRUE', 1, 0)
DF5$correct<-as.numeric(DF5$correct)
DF5 <- DF5 %>% group_by(hash, lesson_name) %>% summarise(mean_score= mean(correct,na.rm = TRUE))
...
```{r}
DF6 <- DF1[,c(7,4)]
DF6$datetime = format(as.Date(as.POSIXct(DF6$datetime, tz="EDT", origin="1970-01-01")),"%m-%d-%Y")
DF6$correct <- ifelse(DF6$correct == 'TRUE', 1, 0)
DF6 <- DF6 %>% group_by(datetime) %>% summarise(mean_correct=mean(correct,na.rm = TRUE))
```
