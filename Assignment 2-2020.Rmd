...
```{r}
plot(S3$login, S3$scores, col= S3$colors, main = "Student Logins vs. Scores")
...
#4
DF3 <- DF2 %>% group_by(hash, lesson_name) %>% summarize(sum(attempt))
...
DF5 <- DF4 %>% group_by(lesson_name) %>% summarise(mean_correct = mean(correct))
