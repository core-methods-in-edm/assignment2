...
1. Create a simulated data set containing 100 students, each with a score from 1-100 representing performance in an educational game. 
The scores should tend to cluster around 75. Also, each student should be given a classification that reflects one of four interest groups: sport, music, nature, literature.
...
S1$scores <- ifelse(S1$scores >= 100, 100, S1$scores)

S1$scores <- round(S1$scores,0)
...
label <- letters[1:10]
S1$breaks <- cut(S1$scores, breaks = 10, labels = label)
```

4. Now using the  package (RColorBrewer; http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) design a pallette and assign it to the groups in your data on the histogram.

```{r}
library(RColorBrewer)
#Let's look at the available palettes in 
display.brewer.all()
#The top section of palettes are sequential, the middle section are qualitative, and the lower section are diverging.
#Make RColorBrewer palette available to R and assign to your bins
S1$colores <- brewer.pal(10,"Set3")
#Use named palette in histogram
hist(S1$scores, breaks=10, col = S1$colores)
```
...
boxplot(scores ~ interest,S1, col = interest.col)
```
...
```{r}
plot(S1$login, S1$scores, main = "Login vs. Scores", col =  interest.col)
```
...
D6 <- data.frame(iris)
pairs(D6)
boxplot(D6$Species,D6$Petal.Length)
```
...
```{r}
DF2 <- select(DF1,lesson_name,hash,attempt)
```
...
```{r}
DF4 <- select(DF1,lesson_name,hash,correct)
```
...
```{r}
score1 <- seq(1,100,1)
score2 <- sample(score1,5725,replace = TRUE)
score3 <- data.frame(score2)
DF1 <- bind_cols(DF1,score3)
DF5 <- DF1 %>% group_by(hash,course_name) %>% summarise(mean_score = mean(score3)) 
```
