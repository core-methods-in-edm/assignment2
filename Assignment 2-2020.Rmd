...
lable<-letters[1:10]
S3$letterbreaks<-cut(S3$score,breaks = 10,labels = lable)
...
```{r}
plot(S3$login, S3$score, col=S3$colors, main = "students login VS. scores")

S3$coll<-ifelse(S3$interest=="music","blue","red")
```
...
plot(Airpassengers,main = "Airline Passengers over time",ylab = "airpassengers")
```
...
table<-table(DF3$lesson_name,col=DF3$lesson_name) 
```
...
DF4$correct <- factor(x=DF4$correct,levels=c('FALSE','TRUE'),labels = c(0,1),exclude = NA)
```
9. Create a new data frame called `DF5` that provides a mean score for each student on each course
```{r}

DF5<- DF1 %>% group_by(course_name,hash)  %>% summarise(mean_score = mean(score))
```
...
DF6$datetime<-as.Date("2000-01-01")
```
