...
plot(S3$login,S3$score,col=S3$col1,main="relationships between logins and scores",xlab="Logins",ylab="Score")
#Alternative
plot(S3$login,S3$score,col=interest.col,main="relationships between logins and scores",xlab="Logins",ylab="Score")
```
...
```{r}
pairs(iris)
#We can run correlation on  Sepal.Length~Petal.Length,  Sepal.Length~Petal.Width,  Sepal.Width~Petal.Length,  Sepal.Width~ Petal.Width,  Petal.Length~Petal.Width.
```
...
```{r}
DF3<-DF3%>%spread(lesson_name, sum_attempt)
```
...
DF5 <- DF5 %>% group_by(hash, course_name) %>% summarise(mean_correct = mean(correct,na.rm = TRUE))
...
DF6$datetime = as.Date(as.POSIXct(DF6$datetime,tz="UTC", origin="1970-01-01"))
DF6$datetime = format(DF6$datetime, format="%m-%d-%Y")
DF6 <- DF6 %>% group_by(datetime) %>% summarise(average.correct = mean(correct,na.rm = TRUE))
```
