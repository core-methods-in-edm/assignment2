...
```{r}
plot(S3$login, S3$score, main = "Login vs. Score")
...
```{r}
DF6 <- data_frame(DF1 [,c("datetime", "correct")])
DF6$correct <- ifelse(DF6$correct == TRUE, 1, 0)
DF6$datetime <- as.Date(as.POSIXct(DF6$datetime, tz = "UTC", origin = "2020-01-01"))
DF6 <- DF6 %>% group_by(datetime) %>% summarise(ave_correct = mean(correct))
```
