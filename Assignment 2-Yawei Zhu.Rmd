...
```{r}
library(readr)
a1 <- read_csv("hudk4050-classes.csv")
library(tidyr)
a2 <- unite(a1, name, "First Name", "Last Name")
a3 <- gather(a2, class, class_number, -name, na.rm = TRUE)
a4 <- a3 %>% select(name,class_number)
names(a4) <- c("from", "to")
a4$count<-1
...
plot(g1, layout=layout.fruchterman.reingold, edge.arrow.size=0.2, vertex.color=as.factor(vertex4050$group), vertex.size=vertex4050$count, vertex.label.size=0.1)
```
