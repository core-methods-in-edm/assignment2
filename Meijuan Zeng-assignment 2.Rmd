...
d3 <- gather(d2, class, class_name, 2: 7, na.rm = TRUE) %>%  select(name, class_name)%>%arrange(name,class_name)%>%na.omit()%>%filter(class_name>0, class_name != "HUDK 4050")

#Delete the unwanted rows: 
d3 <- d3[-c(16:21, 114:119),]
```
...
plot(g,layout = layout.fruchterman.reingold, edge.arrow.size = 0.5, vertex.label.size = 5)
