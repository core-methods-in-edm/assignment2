---
title: "Assignment 2 due on 10/25/2018"
author: "Meijuan"
date: "2018/10/17"
...
# Graph
my_gr <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX)
plot(my_gr,layout=layout.fruchterman.reingold, 
...
#Read the file to create d1:
d1 <- read.csv("HUDK4050-2018-classes.csv", header = TRUE, na.strings = c("", "NA"))

#select name and classes from d1: 
d2 <- select(d1, "Q8": "Q7") %>% unite(name, Q8, Q9, sep = " ") 
class_name <- c("Q1 : Q7")
```

```{r}
#Gather name by the first and last names,all the classes into class_name, arrange their orders and remove the empty rows:
d3 <- gather(d2, class, class_name, 2: 7, na.rm = TRUE) %>%  select(name, class_name)%>%arrange(name,class_name)%>%na.omit()

#Delete the unwanted rows: 
d3 <- d3[-c(22:27, 152:157),]

#remove the spaces in class_name:
d3$class_name <- gsub(' ', '', d3$class_name)
d3 <- filter(d3, class_name > 0, class_name != "HUDK4050")

```

```{r}
#Now spread the table:
d3$count <- 1
d4 <- spread(d3,class_name, count)
rownames(d4) <- d4$name
d4$name <-NULL
d4 <- ifelse(is.na(d4),0,1)
#Convert d5 to create person-class matrix 
d5 <-as.matrix(d4)
#Transpose matrix to produce a person-person matrix
d6 <- d5 %*% t(d5)
diag(d6) <- NA
```
...
library(igraph)
library(klaR)
g <- graph.adjacency(d6, mode = "undirected", weighted = TRUE)
g <- simplify(g, remove.multiple = TRUE, remove.loops = TRUE)
#Use clustering on person-class matrix and show different colors on different groups which share the same classes 
modes <- kmodes(d5, 8, iter.max = 10, weighted = FALSE)
V(g)$color <- modes$cluster
plot(g,layout=layout.fruchterman.reingold, vertex.size = 7)
#betweeness centrality
betweenness(g)
degree(g)
#show who is the most central
max(betweenness(g))
```  
