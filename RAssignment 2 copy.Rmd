...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=(EDGE$count*2))
```
...
plot(g,layout=layout.fruchterman.reingold,vertex.size=(EDGE$count*3),edge.arrow.size=.4,edge.length=0.6,vertex.label.size=0.1)
```
...
```{r}
DS1<-read.csv("hudk4050-classes.csv")
library(tidyr)
DS2<-unite(DS1, Student,First.Name:Last.Name)
DS2$'Class.1'[34] <- "HUDK 4050"
DS2$'Class.2'[34] <- "HUDM 4050"
DS2$'Class.3'[34] <- "HUDM 5026"
DS2$'Class.4'[34] <- "HUDM 5126"
#DS6<-col_number(Class.1)
DS3<- DS2 %>% gather(ClassNumber,Class,Class.1:Class.6,na.rm=TRUE)
library(readr)
library(dplyr)
DS4<-select(DS3, -ClassNumber)
DS5<-table(DS4$Student,DS4$Class)
M1<-as.matrix(DS5)
M2<-t(M1)
M3<-M1 %*% M2
library(igraph)
...
P2<-plot(P1)
centrality <- centr_degree(P1, mode = c("all"))
P3 <- plot(graph_from_adj_list(centrality))
```
