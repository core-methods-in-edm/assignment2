...
```{r}
hist(S3$Score, main ="Histogram of Sample", xlab ="Score", breaks = 10)
...
S3$breaks <- cut(S3$Score, breaks = 10, labels = category)
```
...
interest.col <- brewer.pal(4, "Set1")

boxplot (Score ~ interest, S3, col = interest.col, xlab= "Interest")
```
...
```{r}
plot(S3$Login ~ S3$Score, col = interest.col, main = "Plot of Logins Vs Scores", xlab = "Score", ylab = "Logins")
```
...
```{r}
Swirldata <- read.csv("swirl-data.csv", header = TRUE)
library(dplyr)
DF1 <- data.frame (Swirldata)
```
3. Create a new data frame that only includes the variables `hash`, `lesson_name` and `attempt` called `DF2`
```{r}
DF2 <- data.frame(DF1$lesson_name, DF1$attempt, DF1$hash)
```
...
```{r}
DF3 <- DF1 %>% group_by (lesson_name, hash) %>% summarise(sum_attempts = sum(attempt))
```
...
```{r}
library(tidyr)
CDF3 <- spread(DF3, lesson_name, sum_attempts)
```
...
```{r}
library(dplyr)
DF5a <- DF1 %>% select(hash, course_name, correct)
DF5a$correct <- ifelse(DF5a$correct == "TRUE", "1", "0")
DF5 <- DF5a %>% group_by(hash) %>% summarise(avg.score = mean(DF5a$correct))
```
...
```{r}
DF6a <- DF1 %>% select(datetime)
round(DF6a)
#DF6 <- extract(DF6a, DF1$datetime, into = c("Day", "Month", "Year", "Time"), "(.{2})(.{2})(.{2})(.{4})")

DF6 <- cbind(DF6a, read.csv(text=sub("(.{2})(.{2})(.{2})(.{4})", "\\1,\\2,\\3,\\4",
     DF6a$datetime), header=FALSE, col.names = c("Day", "Month", "Year", "Time")))
```
